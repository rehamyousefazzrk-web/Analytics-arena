...
/* ------------------------------------------------------------------
   SESSION SETUP — everything the trainer can change from the host page
   (⚙️ Session tab). Whatever is saved there overrides these defaults,
   so this file only needs editing if you want new defaults for a fresh
   database.
------------------------------------------------------------------- */
const SETUP = {
  name: "Analytics Arena",
  tagline: "Live session game",
  accent: "#1A9FEF",
  logo: "",                                   // uploaded from the host page
  emojis: EMOJIS,
  flag: { R: { label: "Red flag", emoji: "\uD83D\uDEA9" }, G: { label: "Green flag", emoji: "\u2705" } },
  games: { say: true, box: true, boss: true },
  scoring: { base: 100, speed: 50, poll: 0 }, // speed = max % bonus for answering fast
  timers: { rg: 20, mcq: 30, mcqLong: 40, text: 60 },
  limits: { text: 1000 },          // how many letters a written answer can hold
  // The order here is the order of the session (the trainer can reorder it live).
  rounds: ["1", "RATES", "2", "PULSE", "3", "QB", "4", "VOLT", "F", "BANK"].map(key => ({ key, ...ROUNDS[key] })),
};

module.exports = { SETUP, TEAMS, DEFAULT_TEAMS, EMOJIS, ROUNDS, RG, SAY, SAY_RUBRIC, BOXES, BOX_RUBRIC, BOSS_CASE, BOSS_Q, DROP_OPTIONS, MAX };
